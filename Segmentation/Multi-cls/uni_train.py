...
def train_net(net, device, args):
    ...
    for ki in range(args.K):
        print("%d Fold:" % (ki + 1))
        # 加载训练集
        train_dataset = MultiHeartDataset(args.data_path, train=True, ki=ki, k_folds=args.K, typ='train',
        ...
        val_dataset = MultiHeartDataset(args.data_path, train=True, ki=ki, k_folds=args.K, typ='val',
                                        img_cha=param.img_cha, transforms=data_transform)
