...
def train_net(net, device, args):
    ...
    num_workers = min([os.cpu_count(), args.batch_size if args.batch_size > 1 else 0, 4])  # 8-->4
    
    for ki in range(args.K):
        print("%d Fold:" % (ki + 1))
        # 加载训练集
        train_dataset = HeartDataset(args.data_path, train=True, ki=ki, k_folds=args.K, typ='train',
        ...
        val_dataset = HeartDataset(args.data_path, train=True, ki=ki, k_folds=args.K, typ='val',
                                   img_cha=param.img_cha, transforms=data_transform)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', type=int, default=param.epochs, help='Number of epochs to train.')
    parser.add_argument('--val_epoch', type=int, default=5, help='Number of epochs to val.')
    parser.add_argument('--batch_size', type=int, default=8)  # 8-->4
    parser.add_argument('--K', type=int, default=5, help='Number of cross validation')
    parser.add_argument('--lr', type=float, default=1e-3, help='Learning rate initial')  # lr -3
    parser.add_argument('--lrf', type=float, default=0.01, help='Scheduler Learning rate modify')
    parser.add_argument('--lf', type=float, default=1e-4, help='Learning rate final')  # lrf -4
